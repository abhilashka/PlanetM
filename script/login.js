...
//username validation  for login page
  function check_user() {
    ...
    let expression =
      /^[6-9]\d{9}|([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;
...
//   performing validations on keyup
...
// Login Form starts here
  ...
  $("#login_form").submit(function (e) {
    ...
    if (error_user === false && error_password === false) {
      ...
      exprss = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;           //check for email
      const myUrl = new URL("http://localhost:3000/users/");
      if ($("#username").val().match(exprss)) {                 //search parameters 
